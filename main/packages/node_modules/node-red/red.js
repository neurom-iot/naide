...
if (parsedArgs.help) {
    console.log("NAIDE v"+RED.version());
...
if (parsedArgs.settings) {
    ...
    if (fs.existsSync(path.join(process.env.NODE_RED_HOME,".config.json"))) {
        ...
        settingsFile = path.join(process.env.HOMEPATH,".naide","settings.js");
    } else {
        var userDir = parsedArgs.userDir || path.join(process.env.HOME || process.env.USERPROFILE || process.env.HOMEPATH,".naide");
...
try {
    ...
    if (err.code == "unsupported_version") {
        ...
        console.log("NAIDE requires Node.js v8.9.0 or later");
    } else {
...
function basicAuthMiddleware(user,pass) {
    ...
    var checkPasswordAndCache = function(p) {
        ...
        // a restart of NAIDE. This means we can cache the provided crypted
