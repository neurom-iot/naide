...
function EventSource (url, eventSourceInitDict) {
  var readyState = EventSource.CLOSED
  ...
  self.shouldReconnect = eventSourceInitDict.reconnect !== false

  function onConnectionClosed (err) {
    if (err) _emit('error', new Event(err))
  ...
  function connect () {
    ...
    req = (isSecure ? https : http).request(options, function (res) {
      ...
      res.on('close', function () {
        ...
        onConnectionClosed(new Error('connection closed'))
      })

      res.on('end', function () {
        res.removeAllListeners('close')
        res.removeAllListeners('end')
        onConnectionClosed(new Error('connection ended'))
      })
...
EventSource.prototype.varructor = EventSource; // make stacktraces readable
